...
echo $allips
echo "Execute the following command to whitelist these IPs for enabling CCLM:"
echo  allssh \"modify_firewall -f -r $allips -p 2030,2036,2073,2090 -i eth0\"
echo "Execute the following command to whitelist these IPs for enabling scp access to the cluster:"
echo  allssh \"modify_firewall -f -r $allips -p 2222 -i eth0\"
